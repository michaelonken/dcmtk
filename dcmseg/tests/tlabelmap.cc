...
// 128 x 1024 = 65536 * 2, i.e. space for counting through 16 bit values twice
static const Uint16 NUM_ROWS             = 128;
static const Uint16 NUM_COLS             = 1024;
...
OFTEST(dcmseg_labelmap_8bit_mono2)
{
    ...
    tf.stealFile();
    std::cout << "TODO: Stored to " << temp_fn << std::endl;
...
OFTEST(dcmseg_labelmap_16bit_mono2)
{
    // Make sure data dictionary is loaded
    if (!dcmDataDict.isDictionaryLoaded())
    {
    ...
    // Creation
    DcmSegmentation* seg = createLabelMap(16, DcmSegTypes::SLCM_MONOCHROME2);
    setGenericValues(seg);
    addSharedFGs(seg);
    addFrames<Uint16>(seg, 16);
    addDimensions(seg);
...
OFTEST(dcmseg_labelmap_16bit_palette)
{
    OFCHECK(OFTrue);
    // // Make sure data dictionary is loaded
    // if (!dcmDataDict.isDictionaryLoaded())
...
static void addFrames(DcmSegmentation* seg, const Uint8 bitsAllocated)
{
    ...
    if (fg)
    {
        for (Uint16 frameNo = 1; frameNo <= NUM_FRAMES; frameNo++)
        {
            ...
            Uint32 max_value = (1 << bitsAllocated); // 8 bit: 256, 16 bit: 65536
            for (Uint32 i = 0; i < NUM_PIXELS_PER_FRAME; ++i)
            {
                data[i] = i % max_value;
                std::cout << "TODO: " << data[i] << std::endl;
            }
